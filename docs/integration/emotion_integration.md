...
- 关怀提醒、休息建议、异常提示由 LLM-centered `DecisionPipeline` 判断，并由 `ActionRealizer` 转换成显示、语音、灯光模块的能力 Action。
...
| `src/agent/event/event_builders.py` | 将各后端输出**翻译**为统一 `user_emotion_updated` 载荷；`user_fatigue_updated` 含 `perclos`（眼）、`yawn_ratio`（口部窗口占比）、`yawn_in_window`。 |
