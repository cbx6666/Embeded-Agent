...
class EmotionStateStore:
    ...
    def _record_second_state_locked(
        ...
        *,
        ...
        try:
            ...
            cur.execute(
                """
                INSERT INTO emotion_second_states(ts, emotion, confidence, source, model, updated_at)
                VALUES (?, ?, ?, ?, ?, ?)
    ...
    def query_seconds_by_state(self, *, start_ts: int, end_ts: int) -> dict[str, int]:
        with self._lock:
            ...
            cur.execute(
                """
                SELECT emotion, COUNT(*) AS seconds
                FROM emotion_second_states
    ...
    def query_daily_summary(self, *, day: str) -> dict[str, dict[str, float | int]]:
        with self._lock:
            ...
            cur.execute(
                """
                SELECT emotion, seconds, ratio
                FROM emotion_daily_summary
                WHERE period_key = ?
    ...
    def _apply_delta(
        ...
        cur.execute(
            f"""
            INSERT INTO {summary_table}(period_key, emotion, seconds, ratio, updated_at)
            VALUES (?, ?, ?, 0.0, ?)
            ON CONFLICT(period_key, emotion) DO UPDATE SET
        ...
        cur.execute(
            f"DELETE FROM {summary_table} WHERE period_key = ? AND emotion = ? AND seconds <= 0",
            (period_key, state),
        )
        cur.execute(
            f"""
    ...
    def _ensure_schema(self) -> None:
        ...
        cur.execute(
            """
            CREATE TABLE IF NOT EXISTS emotion_second_states (
        ...
        cur.execute("CREATE INDEX IF NOT EXISTS idx_emotion_second_state_emotion ON emotion_second_states(emotion)")
        cur.execute(
            """
            CREATE TABLE IF NOT EXISTS emotion_daily_summary (
                period_key TEXT NOT NULL,
                emotion TEXT NOT NULL,
                seconds INTEGER NOT NULL,
                ratio REAL NOT NULL DEFAULT 0.0,
                updated_at INTEGER NOT NULL,
                PRIMARY KEY (period_key, emotion)
            )
            """
        )
        cur.execute(
            """
            CREATE TABLE IF NOT EXISTS emotion_daily_totals (
    ...
    def _cleanup_expired_second_states(self, cur: sqlite3.Cursor, now: int) -> None:
        ...
        cur.execute("DELETE FROM emotion_second_states WHERE ts < ?", (threshold_ts,))
