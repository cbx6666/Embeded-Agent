...
# 云端/无本地 WAV 时的默认应答文案；与 DEFAULT_WAKE_ACK_PHRASES 首条保持一致。
DEFAULT_WAKE_ACK_TEXT = "我在，请说。"
DEFAULT_WAKE_ACK_PHRASES: tuple[tuple[str, str], ...] = (
    ("wozai_qingshuo", DEFAULT_WAKE_ACK_TEXT),
    ("zai_de", "在的，请说。"),
    ("ni_shuo", "嗯，你说。"),
    ("wozai", "我在。"),
)
...
class LocalWakeAckPlayer:
    ...
    def pick_clip(self, *, strategy: str = "random") -> WakeAckClip | None:
        """选取一条本地应答（不播放），供统一 TTS 队列使用。"""
        with self._lock:
            if not self._clips:
                return None
